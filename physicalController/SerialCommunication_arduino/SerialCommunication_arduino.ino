...
int leftPin = 7;
int rightPin = 8;
int LEDConnectPin = 11;
int LEDTouchPin = 12;

int upSensor = 0;    // first analog sensor
int downSensor = 0;   // second analog sensor
int leftSensor = 0;    // digital sensor
int rightSensor = 0;
...
void setup()
{
  ...
  pinMode(upPin, INPUT);   // digital sensor is on digital pin 2
  pinMode(downPin, INPUT);   // digital sensor is on digital pin 2
  pinMode(leftPin, INPUT);   // digital sensor is on digital pin 2
  pinMode(rightPin, INPUT);   // digital sensor is on digital pin 2

  pinMode(LEDConnectPin, OUTPUT);
  pinMode(LEDTouchPin, OUTPUT);

  digitalWrite(LEDConnectPin, LOW);
  digitalWrite(LEDTouchPin, LOW);

  establishContact();  // send a byte to establish contact until receiver responds 
}

boolean conLed = false;
...
void loop()
{
  ...
  if (Serial.available() > 0) {
    ...
    if(inByte == 'C'){
      ...
      digitalWrite(LEDTouchPin, HIGH);
    }
    //else{
    //      digitalWrite(LEDPin, LOW);
    //    }

    upSensor = digitalRead(upPin);  
    downSensor = digitalRead(downPin);
    leftSensor = digitalRead(leftPin);
    rightSensor = digitalRead(rightPin);
    //    Serial.write(upSensor);   
    //    Serial.write(downSensor); 
    //    Serial.write(leftSensor); 
    //    Serial.write(rightSensor);     
